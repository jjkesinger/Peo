...
[TestClass]
public class EducationPlanDeductionCalculatorTests
{
    ...
    private readonly EducationPlanDeductionCalculator Target;

    public EducationPlanDeductionCalculatorTests()
    {
        Target = new EducationPlanDeductionCalculator(_employeeElectionLookupService, _employeeContributionLookupService);
    }
    ...
    public async Task CalculateAsync_GivenEmployeeEnrolled_ShouldCalculateAmount()
    {
        ...
        A.CallTo(() => _employeeElectionLookupService
            .GetActiveElections(payroll, ElectionType.Education))
            .Returns([new EmployeeElectionBuilder().WithElectionType(ElectionType.Education).Build()]);

        A.CallTo(() => _employeeContributionLookupService
            .GetEmployeeContributionsAsync(payroll, ElectionType.Education))
            .Returns([new EmployeeContributionBuilder()
                .WithDeductionTaxType(DeductionTaxType.PreTax)
                .WithRate(.2m)
                .WithElectionType(ElectionType.Education)
                .Build()]
            );
