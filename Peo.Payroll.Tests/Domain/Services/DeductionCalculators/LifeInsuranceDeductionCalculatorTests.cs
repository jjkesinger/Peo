...
[TestClass]
public class LifeInsuranceDeductionCalculatorTests
{
    ...
    private readonly LifeInsuranceDeductionCalculator Target;

    public LifeInsuranceDeductionCalculatorTests()
    {
        Target = new LifeInsuranceDeductionCalculator(_employeeElectionLookupService, _employeeContributionLookupService);
    }
    ...
    public async Task CalculateAsync_GivenEmployeeEnrolled_ShouldCalculateAmount()
    {
        ...
        A.CallTo(() => _employeeElectionLookupService
            .GetActiveElections(payroll, ElectionType.Life))
            .Returns([new EmployeeElectionBuilder().WithElectionType(ElectionType.Life).Build()]);

        A.CallTo(() => _employeeContributionLookupService
            .GetEmployeeContributionsAsync(payroll, ElectionType.Life))
            .Returns([new EmployeeContributionBuilder()
                .WithDeductionTaxType(DeductionTaxType.PreTax)
                .WithRate(.2m)
                .WithElectionType(ElectionType.Life)
                .Build()]
            );
