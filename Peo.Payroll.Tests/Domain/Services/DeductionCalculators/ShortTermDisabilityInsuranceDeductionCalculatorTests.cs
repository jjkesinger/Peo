...
[TestClass]
public class ShortTermDisabilityInsuranceDeductionCalculatorTests
{
    ...
    private readonly ShortTermDisabilityInsuranceDeductionCalculator Target;

    public ShortTermDisabilityInsuranceDeductionCalculatorTests()
    {
        Target = new ShortTermDisabilityInsuranceDeductionCalculator(_employeeElectionLookupService, _employeeContributionLookupService);
    }
    ...
    public async Task CalculateAsync_GivenEmployeeEnrolled_ShouldCalculateAmount()
    {
        ...
        A.CallTo(() => _employeeElectionLookupService
            .GetActiveElections(payroll, ElectionType.Std))
            .Returns([new EmployeeElectionBuilder().WithElectionType(ElectionType.Std).Build()]);

        A.CallTo(() => _employeeContributionLookupService
            .GetEmployeeContributionsAsync(payroll, ElectionType.Std))
            .Returns([new EmployeeContributionBuilder()
                .WithDeductionTaxType(DeductionTaxType.PreTax)
                .WithRate(.2m)
                .WithElectionType(ElectionType.Std)
                .Build()]
            );
